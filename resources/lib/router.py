...
def run(argv):
    ...
    if a == 'info_and_play':
        ...
        if len(src) == 1:
            # Si solo hay 1 servidor (sea Mail.ru o Dzen), lo elegimos directamente
            tipo, link = src[0]
            if tipo == 'Mail.ru':
                selected_url = resolve_mailru(link)
            elif tipo == 'Dzen':
                selected_url = resolve_dzen(link)
        elif len(src) > 1:
            # Si hay varios servidores, mostramos un selector en Kodi
            options = [f"▶️ Reproducir en {tipo}" for tipo, link in src]
            dialog = xbmcgui.Dialog()
            idx = dialog.select("Selecciona Servidor", options)
            if idx != -1:
                tipo, link = src[idx]
                if tipo == 'Mail.ru':
                    selected_url = resolve_mailru(link)
        ...
        if selected_url:
            xbmcplugin.setResolvedUrl(HANDLE, True, xbmcgui.ListItem(path=selected_url))
        return
